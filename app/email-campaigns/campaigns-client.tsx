...
export function CampaignsClient({
  ...
  const selectableVisible = filteredCompanies.filter((item) => !item.blocked)
  const selectedCompanies = companies.filter((item) => selected.includes(item.id))
  const selectedEmails = new Set(selectedCompanies.map((item) => item.email.trim().toLowerCase()).filter(Boolean))
  const duplicateCount = Math.max(0, selectedCompanies.length - selectedEmails.size)
  ...
  const selectVisible = () => {
    ...
    setSelected(selectableVisible.slice(0, 25).map((item) => item.id))
  }

  const createDraft = () => startTransition(async () => {
    if (!templateId) {
      setMessage('Choisissez un modèle Brevo avant de créer le brouillon.')
      return
    }
  ...
  const showPreview = () => startTransition(async () => {
    if (!templateId) {
      setMessage('Choisissez d’abord un modèle Brevo.')
      return
    }
    const companyId = selected[0]
    if (!companyId) {
      setMessage('Sélectionnez au moins un office pour générer un aperçu personnalisé.')
      return
    }
    const result = await previewCampaignEmailAction({ templateId, companyId })
    ...
    <div className="rounded-xl border border-border bg-card p-6">
      ...
      <div className="mt-5">
        ...
        <div className="mt-3 rounded-lg border border-border">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-muted px-3 py-2 text-sm">
            <span>
              {selected.length} office(s) sélectionné(s)
              {duplicateCount > 0 ? ` · ${duplicateCount} email(s) en doublon seront exclus` : ''}
            </span>
      ...
      <div className="mt-6">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <Button variant="outline" onClick={showPreview} disabled={pending || !templateId || selected.length === 0}>
            <Eye className="mr-2 h-4 w-4" />Aperçu du mail
          </Button>
          <Button variant="outline" onClick={sendTest} disabled={pending || !templateId || !testRecipientEmail}>
      ...
      <div className="mt-6 rounded-lg border border-border bg-muted/40 p-4">
        ...
        <div className="mt-2 grid gap-2 text-sm md:grid-cols-2">
          ...
          <p>Destinataires sélectionnés : <span className="font-medium">{selected.length}</span></p>
        </div>
