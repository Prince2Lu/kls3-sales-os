...
export function CampaignsClient({ campaigns, recipients, companies }: { campaigns: EmailCampaign[]; recipients: EmailRecipient[]; companies: SelectableCompany[] }) {
  ...
  const send = (id: string) => {
    ...
    startTransition(async () => {
      ...
      setMessage(result.success ? 'Campagne transmise à Brevo.' : result.error ?? 'Erreur')
    })
  }
  ...
  return <div className="space-y-6">
    <div className="rounded-xl border border-border bg-card p-6">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Un seul destinataire par office : contact décideur avec email direct, sinon email générique de l’étude. Limite 300.</p>
      ...
      <div className="mt-4 rounded-lg border border-border">
        <div className="flex items-center justify-between border-b border-border bg-muted px-3 py-2 text-sm"><span>{selected.length} office(s) sélectionné(s)</span><button className="text-accent" onClick={() => setSelected(companies.filter((item) => !item.blocked).slice(0, 300).map((item) => item.id))}>Tout sélectionner</button></div>
        <div className="max-h-64 overflow-auto">{companies.map((company) => <label key={company.id} className="flex items-center gap-3 border-b border-border px-3 py-2 text-sm last:border-0">
          <input type="checkbox" checked={selected.includes(company.id)} disabled={company.blocked} onChange={(event) => setSelected((current) => event.target.checked ? [...current, company.id] : current.filter((id) => id !== company.id))} />
    ...
    <div className="rounded-xl border border-border bg-card p-6">
      ...
      {campaigns.length === 0 ? <p className="mt-2 text-sm text-muted-foreground">Aucune campagne.</p> : <div className="mt-4 space-y-3">{campaigns.map((campaign) => {
        ...
        return <div key={campaign.id} className="rounded-lg border border-border p-4">
          ...
          {campaign.status === 'DRAFT' && <Button onClick={() => send(campaign.id)} disabled={pending}><Send className="mr-2 h-4 w-4" />Envoyer via Brevo</Button>}</div>
